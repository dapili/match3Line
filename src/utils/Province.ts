namespace unfsource {
	class Province {
		...
		private static _instance: Province;
		public static get instance(): Province {
			if (!this._instance) {
				this._instance = new Province();
			}
		...
		public getCH(province: string, city: string) {
			if (province) {
			...
			let i = 0;
			let j = 0;
			for (; i < this._pinyin.length; i++) {
				if (province == this._pinyin[i].name) {
					for (; j < this._pinyin[i].cities.length; j++) {
						if (city == this._pinyin[i].cities[j]) {
							break;
						}
					}
					break;
			...
			if (this._ch[i]) {
				if (this._ch[i].cities[j]) {
					return this._ch[i].name + " " + this._ch[i].cities[j];
				}
				else {
					return this._ch[i].name;
				}
			}
			else {
